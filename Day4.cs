...
public class Day4
{
    public void Run()
    {
        string[] rows = Input.Split(new string[] { Environment.NewLine }, StringSplitOptions.None);
        var columns = rows[0].Length;
        var numberOfRows = rows.Length;

        char[,] xmasArray = new char[numberOfRows, columns];
        ...
        for(var row = 0; row < rows.Length; row++)
        {
            ...
            {
                var charToAdd = rows[row][index];
                xmasArray[row, col] = charToAdd;
                //Console.WriteLine($"row {row}, col {col}, char {charToAdd}");
                index++;
            }
        } 
    }
